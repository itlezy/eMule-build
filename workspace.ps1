...
function Apply-Patch([string]$SubmoduleDir, [string]$PatchFile) {
    $patch = Join-Path $Root "patches\$PatchFile"
    $repo = Join-Path $Root $SubmoduleDir
    & git -C $repo apply --check --ignore-whitespace $patch 2>$null
    $canApply = ($LASTEXITCODE -eq 0)
    if (-not $canApply) {
        & git -C $repo apply --check --ignore-whitespace --reverse $patch 2>$null
        if ($LASTEXITCODE -eq 0) {
            Write-Host "  Skipping $PatchFile (already applied)" -ForegroundColor DarkGray
            return
        }
        throw "Patch $PatchFile cannot be applied."
    }
    Write-Host "  Applying $PatchFile" -ForegroundColor Cyan
    & git -C $repo apply --3way --ignore-whitespace $patch
...
function Enable-MbedTlsThreading {
    $src = Join-Path $Root 'eMule\mbedtls\tf-psa-crypto\include\mbedtls\threading_alt.h'
    $dest = Join-Path $Root 'eMule-mbedtls\tf-psa-crypto\include\mbedtls\threading_alt.h'
    $config = Join-Path $Root 'eMule-mbedtls\tf-psa-crypto\include\psa\crypto_config.h'
    if (-not (Test-Path -LiteralPath $dest)) { Copy-Item -LiteralPath $src -Destination $dest }
    $content = [IO.File]::ReadAllText($config)
    if ($content -notmatch '(?m)^#define MBEDTLS_THREADING_C') {
        $content = $content.Replace('//#define MBEDTLS_THREADING_ALT', '#define MBEDTLS_THREADING_ALT   /* eMule: custom Windows CRITICAL_SECTION impl in threading_alt.h */')
        $content = $content.Replace('//#define MBEDTLS_THREADING_C', '#define MBEDTLS_THREADING_C     /* eMule: required for THREADING_ALT */')
...
function Run-Setup {
    ...
    foreach ($pair in @(
        ...
        @('eMule-ResizableLib','resizablelib-master.patch'),
        @('eMule-zlib','zlib-v1.3.2.patch'),
        @('eMule-mbedtls','mbedtls-mbedtls-4.0.0.patch')
    )) { Apply-Patch $pair[0] $pair[1] }
...
function Build-Libs {
    ...
    $defs = @('cryptopp','id3lib','miniupnp','ResizableLib','zlib','mbedtls') | ForEach-Object {
        $info = $Projects[$_]
        [pscustomobject]@{ Name=$_; Kind=$info.Kind; Path=(Join-Path $Root $info.Path); Build=$(if ($info.Contains('Build')) { Join-Path $Root $info.Build } else { $null }); Out=(Split-Path -Parent (Get-OutputPath $_ $Config)) }
    }
...
function Run-Binary {
    ...
    $launch = {
        ...
        if ($Local) {
            ...
            foreach ($path in @($home, (Join-Path $home 'AppData_Roaming'), (Join-Path $home 'AppData_Local'))) { if (-not (Test-Path -LiteralPath $path)) { $null = New-Item -ItemType Directory -Path $path -Force } }
...
switch ($Command) {
    ...
    'build-app' { $Project = 'eMule'; $r = Get-EnvReport 'build-app' $Config $Project; Show-Report $r; Build-ProjectInternal $r 'eMule' $Config }
    'build-all' { Build-Libs; $r = Get-EnvReport 'build-app' $Config 'eMule'; Show-Report $r; Build-ProjectInternal $r 'eMule' $Config }
    'build-project' { $intent = if ($Project -eq 'eMule') { 'build-app' } else { 'build-project' }; $r = Get-EnvReport $intent $Config $Project; Show-Report $r; Build-ProjectInternal $r $Project $Config }
    'open-solution' { $r = Get-EnvReport 'open-solution' $Config 'eMule'; Show-Report $r; Start-Process -FilePath $r.Tools.DevEnv -ArgumentList @(Join-Path $Root $Projects.eMule.Open) | Out-Null }
    'open-project' { $r = Get-EnvReport 'open-project' $Config $Project; Show-Report $r; Start-Process -FilePath $r.Tools.DevEnv -ArgumentList @(Join-Path $Root $Projects[$Project].Open) | Out-Null }
    'run-binary' { Run-Binary }
    'package' { $r = Get-EnvReport 'package' 'Release' 'eMule'; Show-Report $r; $zip = Join-Path $Root 'eMule0.72a-broadband_x64-snapshot.zip'; if (Test-Path -LiteralPath $zip) { Remove-Item -LiteralPath $zip -Force }; Invoke-Native -Exe $r.Tools.Tar -ArgumentList @('-a','-c','-C',(Join-Path $Root 'eMule\srchybrid\x64\Release'),'-f',$zip,'eMule.exe') -Label 'tar package' -WorkDir $null }
    'clean-config' { $r = Get-EnvReport 'clean-config' $Config 'eMule'; Show-Report $r; $path = Join-Path $Root "eMule\srchybrid\x64\$Config\config"; if (Test-Path -LiteralPath $path) { Remove-Item -LiteralPath $path -Recurse -Force } }
}
