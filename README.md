...
Finally you should be ready to go, this last script `003_build_MSBuild_ALL_libs.cmd` builds all the libraries sequentially in dependency order (zlib first, then libpng, CxImage, etc.). This ensures that each library has its dependencies available before being built. As last step you will have to build eMule `build_MSBuild_eMule.cmd` as of course it depends on all the libraries that you just built.
